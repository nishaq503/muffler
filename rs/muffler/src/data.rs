...
pub fn create_windows(samples: &Array2<f32>, window_size: usize, stride: usize) -> (Vec<ArrayView2<f32>>, Vec<usize>) {
    let sample_len = samples.ncols();
    let window_starts = if (sample_len - window_size) % stride == 0 {
        (0..=(sample_len - window_size)).step_by(stride).collect::<Vec<_>>()
    } else {
    ...
    let train = windows
        ...
        .map(|w| {
            if index == 0 {
                ...
                ndarray::concatenate(Axis(1), &[pre_index, post_index])
                    .unwrap_or_else(|_| unreachable!("We made the slices, so they should have the correct shape."))
            }
        })
        .collect::<Vec<_>>();
...
pub fn reassemble(targets: Array2<f32>, out_shape: (usize, usize), window_starts: &[usize]) -> Array2<f32> {
    ...
    for (i, &window_start) in window_starts.iter().enumerate() {
        ...
        let mut reassembled_slice = reassembled.slice_mut(s![.., window_start..window_end]);
    ...
    for (mut col, count) in reassembled.axis_iter_mut(Axis(1)).zip(counts.iter().map(|&c| c as f32)) {
        col /= count;
    }
...
mod tests {
    ...
    fn test_create_windows() {
        ...
        let (windows, starts) = create_windows(&samples, window_size, stride);
        assert_eq!(windows.len(), starts.len());
        let expected = vec![array![[1., 2., 3.], [6., 7., 8.]], array![[3., 4., 5.], [8., 9., 10.]]];
        assert_eq!(windows, expected);
    }
    ...
    fn test_windows_to_train() {
        ...
        let windows = [array![[1., 2., 3.], [6., 7., 8.]], array![[3., 4., 5.], [8., 9., 10.]]];
        let windows = windows.iter().map(ArrayBase::view).collect::<Vec<_>>();
    ...
    fn test_reassemble_large() -> Result<(), String> {
        ...
        let samples = (0..(num_samples * sample_len))
            ...
            .collect::<Vec<f32>>();
        let samples = Array2::from_shape_vec(out_shape, samples).map_err(|e| e.to_string())?;
